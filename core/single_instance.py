...
Файл-замок при выходе НЕ удаляется: снятие блокировки делает закрытие
дескриптора, а удаление файла давало гонку (unlink → вторая копия открывала
новый inode и тоже считала себя единственной).
    ...
    def release(self):
        ...
        if self._fh:
            ...
            try:
                ...
                pass
            self._fh = None
            # Файл НЕ удаляем (P2-2). Он крошечный (несколько цифр PID), а
            # удаление открывало гонку: вторая копия успевала открыть путь уже
            # ПОСЛЕ unlink и блокировала новый файл (другой inode) — обе копии
            # считали себя единственными. Блокировка снимается самим
            # закрытием дескриптора, а сам файл пусть лежит: он и есть «замок».
        self._locked = False
