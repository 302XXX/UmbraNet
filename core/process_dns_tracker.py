...
class DnsProcessTracker:
    ...
    def status_hint(self) -> str:
        """Подсказка «что делать», если трекер выключен из-за PowerShell (H4)."""
        blocked = self._powershell_unavailable()
        if blocked and not self._available:
            try:
                import win_shell
                return win_shell.POWERSHELL_HINT
            except ImportError:
                return ""
        return ""
    ...
    @staticmethod
    def _powershell_unavailable() -> str:
        """Почему трекер не может работать — человеческим языком (пункт H4).

        Трекер читает события Windows через PowerShell (Get-WinEvent). Замены у
        этого пути нет — но человек должен видеть причину, а не «трекер недоступен».
        """
        try:
            import win_shell
        except ImportError as exc:
            log.debug("win_shell недоступен: %s", exc)
            return ""
        if win_shell.powershell_available():
            return ""
        return win_shell.human_error(win_shell.powershell_status().get("reason", "error"))
    ...
    def _poll_once(self):
        ...
        self._last_record_id = max(self._last_record_id, max_rid)
    ...
    def _loop(self):
        ...
        blocked = self._powershell_unavailable()
        if blocked:
            # PowerShell недоступен — повторять попытку каждые несколько секунд
            # бессмысленно: трекер по событиям без него не поднимется. Говорим
            # это один раз и понятно, а функция остаётся выключенной.
            self._available = False
            self._last_error = blocked
            log.warning("DnsProcessTracker: %s. Маршрутизация по процессам будет "
                        "работать только для уже известных приложений.", blocked)
            return
