...
def get_provider_health() -> ProviderHealth:
    """Возвращает один потокобезопасно созданный singleton.

    DNS-запросы обслуживаются несколькими потоками, поэтому простой паттерн
    ``if _instance is None`` мог создать два трекера одновременно на старте.
    """
    global _instance
