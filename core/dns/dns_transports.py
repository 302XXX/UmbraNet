...
def query_dot(server_ip: str, request, timeout: float = 5.0,
    ...
    if not verify:
        # Явное отключение проверки оставляем для пользовательского режима
        # tls_verify=false, но по умолчанию никогда не деградируем до
        # непроверенного сертификата только из-за отсутствующего hostname.
        ctx.check_hostname = False
        ctx.verify_mode = ssl.CERT_NONE
    effective_hostname = server_hostname or server_ip
    ...
    try:
        ...
        with ctx.wrap_socket(
            raw,
            server_hostname=effective_hostname if verify else (server_hostname or None),
        ) as tls:
...
async def _query_doq_async(server_ip, request, timeout, port,
    ...
    # Даже при подключении к IP сохраняем hostname для SNI/проверки. При
    # verify=True это позволяет проверить IP SAN, вместо непреднамеренного
    # отключения проверки сертификата.
    config.server_name = server_hostname or server_ip
