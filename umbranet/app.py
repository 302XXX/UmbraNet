...
class GlowContainer(QWidget):
    ...
    def _rebuild_sprites(self):
        """Преeндерит туманности в маленькие QPixmap-спрайты.
        ...
        # Сине-бирюзовая туманность левого нижнего угла
        def nebula_bl(p, s):
            grad = QRadialGradient(s * 0.5, s * 0.5, 450)
            grad.setColorAt(0, QColor(52, 220, 240, 14))     # бирюзовый (ACCENT3)
            grad.setColorAt(0.5, QColor(91, 155, 255, 8))    # синий (ACCENT2)
            grad.setColorAt(1, QColor(0, 0, 0, 0))
            p.setBrush(grad)
            p.setPen(Qt.NoPen)
            p.drawEllipse(0, 0, s, s)
        ...
        self._nebula_bl = sprite(600, nebula_bl)
        self._sprites_dpr = dpr

    def paintEvent(self, event):
        # Заливка + два blit'а. Спрайты не зависят от размера окна, поэтому
        # resize больше не требует пересборки чего-либо.
        dpr = self.devicePixelRatioF()
        ...
        p.drawPixmap(-200, h - 400, self._nebula_bl)   # якорь: левый нижний угол
        p.end()
